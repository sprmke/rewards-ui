<template lang="pug">
	.profile-container.section.full-height
		.container
			.title-container
				h1.main-header.text-center.mb-5 Profile Details
			|
			.card.profile-content(:class="{'loading h-400': isAPILoading}")
				.card-body
					.top
						.image-container
							img.card-img-top(:src="userImage" :alt="'User image'")
						.user-details
							.form-group.mb-0
								p.label Name:
								p.value {{ user.name }}
							.form-group.mb-0
								...
								p.value {{ user.email }}
					.bottom
						h5.list-title.text-center.mt-4 Redeemed Rewards
						.list-container(v-if="rewardsList && rewardsList.length > 0")
							.card.rewards(v-for="reward of rewardsList" :key="reward._id")
								.img-container
